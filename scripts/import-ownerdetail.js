/**
 * 导入 ownerdetail CSV：列对齐、reference 用 xxx_wixid 上传并解析 xxx_id，boolean true/false -> 1/0。
...
async function run() {
  ...
  try {
    for (let i = 1; i < lines.length; i++) {
      ...
      const hasData = [row.wix_id, row.ownername, row.email].some(
      ...
      if (row.property_wixid) row.property_id = propertyMap.get(row.property_wixid) || null;
