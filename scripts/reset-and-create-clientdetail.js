/**
 * 删除 clientdetail 表并按新结构重建。
 * 用法：node scripts/reset-and-create-clientdetail.js
 */
...
CREATE TABLE clientdetail (
  ...
  CONSTRAINT fk_clientdetail_pricingplan
...
async function run() {
  ...
  try {
    ...
    await conn.query('DROP TABLE IF EXISTS clientdetail');
    console.log('[reset-clientdetail] Dropped clientdetail');
    await conn.query(createSql);
    console.log('[reset-clientdetail] Created clientdetail');
    await conn.query('SET FOREIGN_KEY_CHECKS = 1');
  } catch (err) {
    console.error('[reset-clientdetail] Error:', err.message);
    process.exit(1);
  } finally {
    await conn.end();
  }
