/**
 * Import pricingplan CSV. Title, corecredit, features->features_json, description, sellingprice, addon->addon_json.
 * Usage: node scripts/import-pricingplan.js [path], default ./pricingplan.csv
 */
...
const csvPath = process.argv[2] || path.join(process.cwd(), 'pricingplan.csv');
const table = 'pricingplan';

const CSV_TO_DB = {
  _id: 'wix_id', ID: 'wix_id', id: 'wix_id',
  title: 'title', Title: 'title',
  corecredit: 'corecredit', Corecredit: 'corecredit',
  features: 'features_json', Features: 'features_json',
  description: 'description', Description: 'description',
  sellingprice: 'sellingprice', Sellingprice: 'sellingprice',
  addon: 'addon_json', Addon: 'addon_json',
...
async function run() {
  ...
  const headerToDb = (h) => {
    ...
    if (t.toLowerCase() === 'features') col = 'features_json';
    if (t.toLowerCase() === 'addon') col = 'addon_json';
  ...
  if (tableColumns.size === 0) {
    console.error('Table "pricingplan" does not exist.');
    await conn.end();
    process.exit(1);
  }
  ...
  try {
    for (let i = 1; i < lines.length; i++) {
      ...
      for (const key of ['features_json', 'addon_json']) {
        if (row[key] != null && typeof row[key] === 'string' && row[key].trim()) {
          try { JSON.parse(row[key]); } catch (e) { row[key] = null; }
        }
      }
      const hasData = [row.wix_id, row.title, row.corecredit, row.sellingprice].some(v => v != null && String(v).trim() !== '');
      if (!hasData) continue;
    ...
    if (inserted === 0 && lines.length > 1) {
      console.warn('No rows inserted. Check CSV headers: Title, corecredit, features, description, sellingprice, addon.');
    }
