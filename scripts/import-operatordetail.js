/**
 * 导入 operatordetail CSV。array/object 存 text；pricingplan 用 pricingplan_wixid 上传并解析 pricingplan_id。
 * 用法：node scripts/import-operatordetail.js [csv_path]
 * 默认 csv_path = ./operatordetail.csv
 */
...
const { syncAll, syncSubtablesFromOperatordetail } = require('../src/services/client-subtables');
...
function parseJsonOrJson5(str, debugLabel) {
  ...
  try {
    ...
    try {
      ...
      if (!repaired) {
        if (debug && debugLabel) console.warn('[import-operatordetail] parse failed (no repair):', debugLabel, 'JSON:', e1.message, 'JSON5:', e2.message);
        return null;
      }
      try {
        ...
        try {
          ...
          if (debug && debugLabel) console.warn('[import-operatordetail] repaired parse failed:', debugLabel, e3.message);
          throw e3;
        }
      }
    }
...
async function run() {
  ...
  if (!fs.existsSync(fullPath)) {
    ...
    console.error('Usage: node scripts/import-operatordetail.js [csv_path]');
    process.exit(1);
  }
  ...
  try {
    for (let i = 0; i < records.length; i++) {
      ...
      if (i === 0 && debugCsv) {
        ...
        console.warn('[import-operatordetail] row1 subtable columns:', sub.map(h => h + '→len ' + (record[h] != null ? String(record[h]).length : 'undef')).join(', '));
      }
      ...
      if (hasSubtableColumns) {
        ...
        if (rawIntegration || rawProfile || rawPricingplandetail || rawCredit) {
          console.log('[import-operatordetail] Synced subtables from operatordetail (integration/profile/pricingplandetail/credit)');
        }
      } else {
        ...
        try {
          ...
          console.warn('[import-operatordetail] JSON/JSON5 parse failed for subtables:', e.message);
        }
        const toArray = (v) => {
        ...
        if (Array.isArray(integrationArr) || Array.isArray(profileArr) || Array.isArray(pricingplandetailArr) || Array.isArray(creditArr)) {
          ...
          if (integrationArr?.length) console.log('[import-operatordetail] Synced client_integration:', integrationArr.length, 'rows');
          if (profileArr?.length) console.log('[import-operatordetail] Synced client_profile:', profileArr.length, 'rows');
          if (pricingplandetailArr?.length) console.log('[import-operatordetail] Synced client_pricingplan_detail:', pricingplandetailArr.length, 'rows');
          if (creditArr?.length) console.log('[import-operatordetail] Synced client_credit:', creditArr.length, 'rows');
        } else if ([rawIntegration, rawProfile, rawPricingplandetail, rawCredit].some(Boolean)) {
          console.warn('[import-operatordetail] No subtable data - run migration 0002 to add integration/profile/pricingplandetail/credit columns, then re-import.');
        }
      }
      ...
      if (tableColumns.has('bukku_saas_contact_id') && process.env.BUKKU_SAAS_API_KEY && process.env.BUKKU_SAAS_SUBDOMAIN) {
        try {
          ...
          if (createRes.ok && createRes.contactId != null) {
            await conn.query('UPDATE operatordetail SET bukku_saas_contact_id = ?, updated_at = ? WHERE id = ?', [createRes.contactId, now, clientId]);
            if (inserted <= 5) console.log('[import-operatordetail] Bukku contact created for', clientId, '→', createRes.contactId);
          }
        } catch (bukkuErr) {
          console.warn('[import-operatordetail] Bukku contact create failed for', clientId, bukkuErr?.message || bukkuErr);
        }
      }
