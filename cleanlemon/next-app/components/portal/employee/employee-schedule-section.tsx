...
const statusConfig = {
  'pending-checkout': { label: 'Pending Checkout', color: 'bg-yellow-100 text-yellow-700' },
...
export default function EmployeeTaskPage() {
  ...
  const todayCompletion = useMemo(() => {
    ...
    const todayJobs = visibleTasks.filter((t) => t.date === today)
    const total = todayJobs.length
    const done = todayJobs.filter((t) => t.status === 'completed').length
  ...
  const handleOpenDoorClick = async (task: Task) => {
    const operatorId = getEmployeeOperatorId()
    if (!operatorId) {
      toast.error('Select operator in the header first.')
      return
    ...
    if (!r?.ok) {
      toast.error(r.reason || 'Could not load locks')
      return
    }
    const targets = r.targets || []
    if (targets.length === 0) {
      toast.info('No smart door is bound for this job.')
      return
    }
    if (targets.length === 1) {
      await runUnlock(task, targets[0].lockDetailId)
      return
    }
    setUnlockState({
      task,
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card>
          <CardContent className="pt-6">
            <p className="text-sm text-muted-foreground">Today completion</p>
            <p className="text-2xl font-semibold">{todayCompletion.pct}%</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <p className="text-sm text-muted-foreground">Completed</p>
            <p className="text-2xl font-semibold">{todayCompletion.done}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <p className="text-sm text-muted-foreground">Total jobs</p>
            <p className="text-2xl font-semibold">{todayCompletion.total}</p>
          </CardContent>
        </Card>
      </div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Estimate Complete</TableHead>
                  <TableHead>Service</TableHead>
                  <TableHead>Status</TableHead>
                  <TableHead className="text-right">Open Door</TableHead>
                ...
                {loading ? (
                  ...
                  filteredTasks.map((task) => (
                    <TableRow key={task.id}>
                      ...
                      <TableCell>{task.staffStartTime || '-'}</TableCell>
                      <TableCell>{task.staffEndTime || '-'}</TableCell>
                      <TableCell>
                        {task.estimateCompleteAt
                          ? task.estimateCompleteAt.replace('T', ' ').slice(0, 16)
                          : '-'}
                      </TableCell>
                      ...
                      <TableCell className="text-right">
                        <Button
                          size="sm"
                          variant="outline"
                          disabled={unlockLoadingId === task.id || unlockSubmitting}
                          onClick={() => handleOpenDoorClick(task)}
                        >
                          <DoorOpen className="h-4 w-4 mr-1" />
                          {unlockLoadingId === task.id ? 'Loading…' : 'Open Door'}
                        </Button>
                      </TableCell>
                      <TableCell className="text-right">
                        <DropdownMenu>
