...
function buildSmartDoorRemarkBlock(p: ClientPortalPropertyDetail | null): { text: string; allNA: boolean } {
  if (!p) return { text: '', allNA: false }
  const perm = (p.smartdoorPassword || '').trim() || 'n/a'
  const mail = (p.mailboxPassword || '').trim() || 'n/a'
  const bt = p.smartdoorGatewayReady ? 'available' : 'n/a'
  const lines = [
    'Smart door status',
    `Permanent password: ${perm}`,
    `Mailbox password: ${mail}`,
    `Bluetooth: ${bt}`,
  ]
  const allNA = perm === 'n/a' && mail === 'n/a' && !p.smartdoorGatewayReady
...
function pickLeadTimeForService(
  cfg: Record<string, unknown> | null | undefined,
  serviceKey: ServiceKey
): string {
  if (!cfg || typeof cfg !== 'object') return 'same_day'
  const globalLt = String((cfg as CleanlemonPricingConfig).leadTime || 'same_day').trim()
  const bySvc = (cfg as CleanlemonPricingConfig).leadTimeByService
  if (bySvc && typeof bySvc === 'object') {
    const per = String((bySvc as Record<string, string>)[serviceKey] || '').trim()
    if (per) return per
  }
  return globalLt
}

type ClientBookingContentProps = {
  /** When true, outer padding matches dialog/sheet parents (default full page padding). */
  embedded?: boolean
  /** Bumps when booking overlay opens — resets wizard steps. */
  bookingSessionKey?: number
}

export function ClientBookingContent({ embedded, bookingSessionKey }: ClientBookingContentProps) {
  const { user } = useAuth()
  ...
  /** Desktop booking dialog: property vs services/date + summary. */
  const [bookingDesktopStep, setBookingDesktopStep] = useState<'property' | 'schedule'>('property')
  /** Mobile sheet: property → schedule (service/date) → summary + confirm. */
  const [bookingMobileStep, setBookingMobileStep] = useState<'property' | 'schedule' | 'summary'>('property')
  ...
  useEffect(() => {
    ...
    setBookingMobileStep('property')
    setBookingDesktopStep('property')
  }, [bookingSessionKey])
  ...
  const smartDoorRemarkPreset = useMemo(() => {
    if (bookingMode === 'single' && propertyDetail) {
      return buildSmartDoorRemarkBlock(propertyDetail)
    }
    if (isBulkLikeBooking && bulkPropertyIds.length > 0) {
      const firstId = bulkPropertyIds[0]
      const p = bulkPropertyDetails[firstId]
      return buildSmartDoorRemarkBlock(p)
    }
    return { text: '', allNA: false }
  }, [bookingMode, propertyDetail, isBulkLikeBooking, bulkPropertyIds, bulkPropertyDetails])

  useEffect(() => {
    if (!smartDoorRemarkPreset.text) return
    setBookingRemark(smartDoorRemarkPreset.text)
  }, [smartDoorRemarkPreset.text])
  ...
  const leadTimeRaw = useMemo(
    () => pickLeadTimeForService(pricingConfigCache, bookingServiceKey),
    [pricingConfigCache, bookingServiceKey]
  )
              ...
              {!isDesktopBooking && bookingMobileStep === 'property' && (
                <div className="mt-auto shrink-0 border-t border-border bg-background pt-4">
                  <Button
                    ...
                    className="w-full bg-primary text-primary-foreground"
                    disabled={!hasBookingSelection}
                    onClick={() => setBookingMobileStep('schedule')}
                  >
                    Next
                  </Button>
                </div>
                ...
                <>
                  ...
                  {!isDesktopBooking && bookingMobileStep === 'schedule' ? (
                    <Button
                      ...
                      variant="ghost"
                      className="mb-2 -ml-2 h-9 justify-start px-2 text-muted-foreground hover:text-foreground"
                      onClick={() => setBookingMobileStep('property')}
                    >
                      <ChevronLeft className="mr-1 h-4 w-4 shrink-0" />
                      Back
                    </Button>
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    {smartDoorRemarkPreset.allNA && hasBookingSelection && showScheduleFields ? (
                      <p className="text-xs text-destructive">
                        Key access is unclear. Please add how staff can collect keys (e.g. mailbox location, who to
                        contact).
                      </p>
                    ) : null}
                  </div>
                  ...
                  {!isDesktopBooking && bookingMobileStep === 'schedule' && (
                    <div className="border-t border-border pt-4">
                      <Button
                        ...
                        onClick={() => setBookingMobileStep('summary')}
                      >
                        Next
                      </Button>
                    </div>
  ...
  return (
    <div className={cn(embedded ? 'space-y-4' : 'p-4 md:p-6 space-y-6')}>
      <div>
        ...
        {propertyGroups.length > 0 ? (
          <div className={cn('max-w-xs space-y-1', !embedded && 'mt-4')}>
