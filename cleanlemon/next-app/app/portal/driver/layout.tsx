...
const fullNavItems = [
  { href: '/portal/driver', label: 'Dashboard', icon: LayoutDashboard },
  { href: '/portal/driver/task', label: 'Tasks', icon: ListTodo },
  { href: '/portal/driver/kpi', label: 'My KPI', icon: BarChart3 },
  { href: '/portal/driver/profile', label: 'Profile', icon: User },
]

const PROFILE_HREF = '/portal/driver/profile'
...
export default function DriverLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (hasDriverBinding) return
    if (pathname === PROFILE_HREF) return
    router.replace(PROFILE_HREF)
  }, [sessionReady, hasDriverBinding, pathname, router])
      ...
      <div className="p-4 border-b border-sidebar-border">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <Badge variant="secondary" className="ml-2 text-xs bg-sidebar-accent text-sidebar-accent-foreground">Driver</Badge>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-lg font-semibold text-foreground">
                {navItems.find((item) => item.href === pathname)?.label || 'Driver Portal'}
              </h1>
            </div>
