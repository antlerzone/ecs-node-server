...
- **Coliving Portal — 进入 Tenant / Owner 时 ensure 业务主档（2026-04）：**
  ...
  - **前端：** Coliving Next（`coliving/next-app`）**`owner-layout-client.tsx`** / **`tenant-layout-client.tsx`** 在挂载 **`OwnerProvider` / `TenantProvider`** 前先调用 **`ensureColivingPortalDetail('owner'|'tenant')`**（`lib/unified-profile-portal-api.ts`）。**demo.colivingjb.com** 无真实 API 基址时为 no-op。
...
- **详细：** [cron-daily-setup-step-by-step.md](./cron-daily-setup-step-by-step.md)、[coliving/next-app/](../coliving/next-app/)。
...
- **Portal 列表「Balance」**：`meterdetail.balance` 与 CNYIOT 一致存的是 **剩余 kWh**，不是 RM。Operator 页展示为 **kWh 为主**；prepaid 时另显示 **≈ RM（rate × kWh）** 作金额参考。例：rate **RM 5/kWh**、充值 **RM 10** → DB **+2 kWh**，正确展示为 **「2 kWh」** 与 **「≈ RM 10.00」**；若仍见 **「RM 2.00」** 是**旧版前端静态资源**，未执行 Next build。**改 `coliving/next-app` 后必须**：仓库根目录 `npm run build:portal`（或 `cd coliving/next-app && npm run build`）再 `pm2 restart portal-next`；**仅 `pm2 restart` 不会更新页面**。
