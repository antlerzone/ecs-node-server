...
> 本文档记录 clientdetail 主表与 4 张子表的导入流程、脚本、以及「从 clientdetail 表自动同步到子表」的用法，下次可直接按此操作。

---

## 分类（主表 vs 子表）

**主表 clientdetail** 存扁平字段；若已跑 migration 0002，还会存 integration/profile/pricingplandetail/credit 四列（JSON text），供服务自动同步到子表。
...
导入时：先写 **clientdetail**，再根据 CSV 四列 JSON **自动写入** 4 张子表（或若表有 4 列则写入主表后由服务从表同步到子表）。
...
1. 本机上传 CSV 到 ECS（PowerShell）：
   ```powershell
   scp -i $HOME\.ssh\malaysia-ecs-key.pem "$env:USERPROFILE\Downloads\clientdetail.csv" ecs-user@47.250.141.3:/home/ecs-user/app/clientdetail.csv
   ```
2. SSH 登录后执行（默认用 `./clientdetail.csv`）：
   ```bash
   cd /home/ecs-user/app
   node scripts/clear-and-import-clientdetail.js
   ```
   或指定 CSV 路径：`node scripts/clear-and-import-clientdetail.js ./clientdetail.csv`

### 方式 B：分步（清空 → 上传 → 导入）

1. **ECS：清空**  
   `node scripts/clear-client-and-subtables.js`
2. **本机：** 从 Wix 导出 ClientDetail（含 integration / Profile / pricingplandetail / credit 四列，标准 JSON 双引号格式），保存为 `clientdetail.csv`，用 scp 上传到 app 目录。
3. **ECS：导入**  
   `node scripts/import-clientdetail.js ./clientdetail.csv`
...
## 从 clientdetail 表同步到子表（无需再依赖 CSV）

- **Migration 0002**：为 clientdetail 增加四列 `integration`, `profile`, `pricingplandetail`, `credit`（TEXT）。  
  执行：`src/db/migrations/0002_clientdetail_subtable_json_columns.sql`（按项目 migration 方式执行一次）。

- **服务** `syncSubtablesFromClientdetail(conn, clientId)`（`src/services/client-subtables.js`）：  
  从 clientdetail 读取该 client 的上述四列 JSON，解析后写入 client_integration、client_profile、client_pricingplan_detail、client_credit。  
  **每次对 clientdetail 做 insert/update 后调用一次**，即可自动更新 4 张子表，无需 trigger、无需再传 CSV。

- **验证 / 重跑同步**：若子表没数据或想按表内 JSON 重写子表，可执行：  
  `node scripts/verify-and-sync-client-subtables.js`  
  会先打 clientdetail 与 4 张子表行数，再对「四列有数据的 client」执行一次从表到子表的同步。
...
| 脚本 | 作用 |
|------|------|
| `clear-client-and-subtables.js` | 仅清空 clientdetail + 4 张子表 |
| `clear-and-import-clientdetail.js [csv]` | 清空 + 导入（默认 `./clientdetail.csv`） |
| `import-clientdetail.js [csv]` | 仅导入，不清空 |
| `verify-and-sync-client-subtables.js` | 查行数 + 从 clientdetail 四列重新同步到子表 |
