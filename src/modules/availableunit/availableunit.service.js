...
async function getClientBySubdomain(subdomain) {
  ...
  const currency = (r.currency && String(r.currency).trim()) ? String(r.currency).trim().toUpperCase() : 'MYR';
...
async function getList(clientId, opts = {}) {
  ...
  const conditions = ['r.client_id = ?', 'r.active = 1', '(r.available = 1 OR r.availablesoon = 1)', '(p.active = 1 OR p.id IS NULL)'];
  ...
  const [propRows] = await pool.query(
    'SELECT id, shortname FROM propertydetail WHERE client_id = ? AND active = 1 ORDER BY shortname ASC LIMIT 500',
    [clientId]
  );
  const properties = [
    { value: 'ALL', label: 'All' },
...
async function getDataPublic(opts = {}) {
  ...
  const conditions = ['r.active = 1', '(r.available = 1 OR r.availablesoon = 1)', '(p.active = 1 OR p.id IS NULL)'];
  const params = [];
  if (propertyId) {
    conditions.push('r.property_id = ?');
    params.push(propertyId);
  ...
  const items = (rows || []).map(r => {
    ...
    return {
      ...
      currency: (r.client_currency && String(r.client_currency).trim()) ? String(r.client_currency).trim().toUpperCase() : 'MYR'
    };
  });

  const propWhere = ['p.active = 1', 'r.active = 1', '(r.available = 1 OR r.availablesoon = 1)'];
  const propParams = [];
  if (keyword.length >= 1) {
...
async function getData(subdomain, opts = {}) {
  ...
  return {
    ...
    clientCurrency: ctx.clientCurrency || 'MYR',
