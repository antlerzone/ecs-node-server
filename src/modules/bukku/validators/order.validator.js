...
const order_child_item_schema = joi.object({
...
const order_form_item_schema = joi.object({
  id: joi.number().optional(),
  ...
  discount: joi.string().max(14).optional(),
  tax_code_id: joi.number().optional(),
  children: joi.array().items(order_child_item_schema).optional()
});
...
const create_order_schema = joi.object({
  ...
  form_items: joi.array().items(order_form_item_schema).min(1).required(),
...
const update_order_schema = create_order_schema.keys({
  number: joi.string().max(50).required()
});

const list_order_schema = joi.object({
...
const update_order_status_schema = joi.object({
