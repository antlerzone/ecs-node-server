...
const sort_by_enum = ['number', 'date', 'contact_name', 'number2', 'description', 'amount', 'created_at'];
...
const create_purchase_refund_schema = joi.object({
...
const update_purchase_refund_schema = create_purchase_refund_schema.keys({
  number: joi.string().max(50).required()
});

const list_purchase_refund_schema = joi.object({
...
const update_purchase_refund_status_schema = joi.object({
  status: joi.string().valid(...status_enum).required(),
