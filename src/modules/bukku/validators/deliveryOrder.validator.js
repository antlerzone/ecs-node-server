...
const transfer_status_enum = ['ALL', 'NOT_TRANSFERRED', 'PARTIAL_TRANSFERRED', 'TRANSFERRED'];
const sort_by_enum = ['number', 'date', 'contact_name', 'number2', 'title', 'description', 'amount', 'created_at'];
const sort_dir_enum = ['asc', 'desc'];

const delivery_order_child_item_schema = joi.object({
...
const delivery_order_form_item_schema = joi.object({
  id: joi.number().optional(),
  ...
  discount: joi.string().max(14).optional(),
  tax_code_id: joi.number().optional(),
  children: joi.array().items(delivery_order_child_item_schema).optional()
});
...
const create_delivery_order_schema = joi.object({
  ...
  form_items: joi.array().items(delivery_order_form_item_schema).min(1).required(),
...
const update_delivery_order_schema = create_delivery_order_schema.keys({
  number: joi.string().max(50).required()
});

const list_delivery_order_schema = joi.object({
...
const update_delivery_order_status_schema = joi.object({
