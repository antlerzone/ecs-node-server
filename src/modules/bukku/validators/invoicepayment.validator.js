...
const create_payment_schema = joi.object({
...
const update_payment_schema = create_payment_schema.keys({
  number: joi.string().max(50).required()
});

const list_payment_schema = joi.object({
...
const update_payment_status_schema = joi.object({
  status: joi.string().valid(...status_enum).required(),
