...
const quote_child_item_schema = joi.object({
...
const quote_form_item_schema = joi.object({
  id: joi.number().optional(),
  ...
  discount: joi.string().max(14).optional(),
  tax_code_id: joi.number().optional(),
  children: joi.array().items(quote_child_item_schema).optional()
});
...
const create_quote_schema = joi.object({
  ...
  form_items: joi.array().items(quote_form_item_schema).min(1).required(),
...
const update_quote_schema = create_quote_schema.keys({
  number: joi.string().max(50).required()
});

const list_quote_schema = joi.object({
...
const update_quote_status_schema = joi.object({
