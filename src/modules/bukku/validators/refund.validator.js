...
const create_refund_schema = joi.object({
...
const update_refund_schema = create_refund_schema.keys({
  number: joi.string().max(50).required()
});

const list_refund_schema = joi.object({
...
const update_refund_status_schema = joi.object({
  status: joi.string().valid(...status_enum).required(),
